...
def resolve_github_credentials(
    ...
    resolved_token = (
        token or os.getenv("GITHUB_TOKEN") or os.getenv("GITHUB_DATA_TOKEN")
    )
...
def list_session_paths(
    repo: str,
    token: str,
    prefix: str = "sessions/xai_as_closure/study2/",
    ref: str = "main",
) -> list[str]:
...
def download_sessions(
    ...
    for remote_path in paths:
        local_path = (out_root / remote_path).resolve()
        if resolved_root not in local_path.parents:
            raise GitHubLoaderError(
                f"Remote session path escapes the extraction directory: {remote_path}"
            )
