...
_META_SIGNALS = frozenset(
    {"ImportId", "Response ID", "_recordId", "Start Date", "startDate"}
)
...
def _detect_pid_column(fieldnames: list[str], preferred: str | None = None) -> str:
...
def stack_and_clean(
    ...
    for i, path in enumerate(paths):
        ...
        rows, stats = load_and_clean(
            path, pid_col=pid_col, source_label=label, keep=keep
        )
