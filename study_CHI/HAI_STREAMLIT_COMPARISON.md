...
| Welcome, fictional-scenario disclosure, advisory AI statement, role screen, policy screen | Adapted as one concise recruitment-brief screen with optional full documents | The current protocol avoids making long document navigation feel like the primary task while keeping the complete sources available. |
...
| Condition-sensitive recommendation button, spinner, and conversational response language | Ported and expanded | LowA/HighA governs the complete frozen recommendation card, not only a wrapper. |
...
| Citation controls | Ported as neutral claim-linked CV/JD/POL locators | In explanation-present cells, citation locators are written into the recommendation passage and compact clickable controls open the complete document at that location. In explanation-absent cells, only the verdict is shown; stable `P0`/`P1` IDs remain deployment keys. |
...
| HIC Stage 2 selectable evidence challenge | Excluded | It is not part of the current explanation × anthropomorphism × forcing workflow and would add an unregistered post-recommendation intervention. |
